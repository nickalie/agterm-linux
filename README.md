...
- **Three restore modes.** Restore the saved layout with fresh shells, re-run the commands captured at quit, or keep the actual primary and split processes alive with the bundled zmx multiplexer. Live sessions are global, need zsh as the account's login shell, and take effect after restarting agterm.
- **Remote sessions.** A session running on another machine can be attached here, split included, over ssh and nothing else. `agtermctl zmx tree HOST` lists what it offers and `agtermctl zmx attach HOST SESSION` grabs one.
...
agtermctl sidebar mode flagged                   # show only the flagged sessions (tree|flagged|toggle)
agtermctl sidebar flagged-layout tree            # nest them under their workspaces in every window (flat|tree|toggle)
...
**Settings ▸ General ▸ Restore sessions** chooses what comes back: **Fresh shells** (the default), **Re-run commands**, or **Live sessions**.

Live mode keeps the process instead of the command. Each local primary and split pane runs inside the bundled zmx multiplexer, so quitting agterm ends the connection to the pane while the process itself keeps running, and the next launch reattaches to it. A build still compiling or an agent halfway through a task is still there. The mode is frozen for the life of the process, so it takes effect after restarting agterm, and it needs zsh as the account's login shell — an unsupported shell falls back to fresh shells and Settings says why. Scratch, overlay and quick terminals stay temporary, and a remote session is never wrapped. `agtermctl zmx list` shows every daemon and the pane holding it, `agtermctl zmx prune` clears unclaimed daemons with no attached clients, `agtermctl zmx kill` destroys one pane's daemon, and `agtermctl restore mode` reads or sets the policy without opening Settings. Switching back to another mode and restarting reaps every detached agterm daemon in that state directory. A clean quit still captures each pane's foreground command as a fallback, so a daemon that did not survive an orderly machine restart comes back running that command rather than a bare shell.

Live restore keeps the running process, usable text and TUI state, and normal terminal colors. The reconstructed screen does not retain inline images, earlier prompt markers, program-changed palette entries, or hyperlink metadata already attached to cells. New output after reattach behaves normally.

Outside live mode, restore reconstructs the structure, not the running processes, and three limitations follow from the design:

1. A restored session re-spawns a fresh login shell in its saved working directory. **Re-run commands** re-runs the command each pane had in the foreground at the last clean quit, the command each pane had in the foreground at the last clean quit, so a gate `ssh`, `tail -f`, or `top` comes back — but it is a re-run, not a reattach: only a single-process command restores faithfully (pipelines and compound lines do not); a force-quit or crash captures nothing, and a capture replays exactly once — the launch that arms it clears it from the state file, so a crash later that session can cost you the restore but never re-runs a command twice; and the programs named in `restore-denylist.conf` (in the config directory, seeded with the terminal multiplexers `tmux`/`screen`/`zellij`, one command name per line) are skipped so they start fresh rather than re-launching — everything else, including `python manage.py runserver` or `node server.js`, is restored. Edit that file to add or remove entries. A per-session, per-pane override can pin what a pane restores, winning over both the captured foreground and the session's own `--command`: `agtermctl session restore "claude --resume <id>" --target <session>` pins a shell line, `--none` pins nothing (the pane comes back as a plain shell), and `--clear` drops the override to fall back to auto-capture. The override is written now and consumed on the next launch — it never touches the running session — and it is sticky: it fires again on every restart until cleared, obeys the same setting but bypasses the denylist (it names its command deliberately), and reads back on `tree` as `restoreCommand` (main pane) / `splitRestoreCommand` (split pane). It exists for commands the capture cannot restore usefully on its own — a captured bare `claude` re-runs as an EMPTY session, and `claude --resume <id> --fork-session` mints a new one on every restart. The fix is a hook that rewrites the override to the live session id on every start, which Claude Code gets from the installed [agent hooks](#agent-status) already: its `SessionStart` pins `claude --resume <id>`, its `SessionEnd` clears the pin when you leave claude deliberately, and any other agent can do the same from its own lifecycle hook. Ownership flips to whoever sets it — write it once by hand and forget, and it stays pinned to a stale id. The pinned value is shell code stored in the window's state file and readable via `tree`, so it must not carry secrets.
