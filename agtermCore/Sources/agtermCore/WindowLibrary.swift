...
public final class WindowLibrary {
    ...
    private func strayWindowFileIDs(indexed: Set<UUID>) -> [UUID]? {
        guard FileManager.default.isExistingDirectory(windowsDirectory),
              let contents = try? FileManager.default.contentsOfDirectory(at: windowsDirectory, includingPropertiesForKeys: nil) else {
            return nil
        }
        return contents
            .filter { $0.pathExtension == "json" }
