...
extension AppStore {
    ...
    /// The window's sessions waiting on the user across ALL workspaces, ignoring the focus/flagged sidebar
    /// filter (unlike `navigableSessions`) so they stay visible with the sidebar hidden. Ordered by
    /// `attentionPrecedes`.
    ///
    /// The filter is `AgentStatus.needsAttention`, so `active` is out: the installed hooks hold a working
    /// agent there almost continuously, and answering a prompt promotes `blocked` → `active`
    /// (`AgentStatus.afterKeystroke`), which together made an attended session stay listed.
