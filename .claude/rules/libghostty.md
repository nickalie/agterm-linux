...
- **Linux Dashboard mirrors live surfaces; it never reparents a `GtkGLArea`.**
  The AppKit/Metal reparenting rules above apply only to the macOS host. Unparenting a GTK Ghostty surface
  unrealizes its `GtkGLArea`, invalidates its GL context, and can leave the terminal blank and
  non-interactive after reattachment.
  The Linux terminal deck therefore uses a stable `GtkOverlay` child per session: normal mode maps only the
  active page, Dashboard mode maps every page while letting none accept input. Each cell displays its source
  through `GtkWidgetPaintable` + `GtkPicture` beneath an opaque, input-owning Dashboard host, so output stays
  live and closing Dashboard only changes visibility and targeting state — no PTY resize, unlike macOS.
