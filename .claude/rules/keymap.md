...
- **Linux caches the parsed keymap PER WINDOW CONTROLLER, so every explicit reload MUST go through
  ...
  macOS keeps one app-wide `SettingsModel.keymap`, so a reload there is app-global for free. The GTK port
  caches `keymap`, `resolvedBuiltinChords`, and `customCommandEngine` on EACH `AppController`, rebuilt only
  by `reloadKeymapDiagnostics()` (`KeymapDispatch.swift`), so rebuilding one controller leaves every other
  window on the previous bindings. The seam (`WindowManager.swift`) maps `reloadKeymapDiagnostics()` over
  `gWindows.values`, reduces the identical per-window counts with `max() ?? 0`, reports parse errors once,
  and returns the count. All four explicit-reload sites route through it: the palette's Reload Keymap row,
...
- Three details the Linux seam depends on. Startup is deliberately NOT fanned out: window construction
  ...
  `keymapReloadToast(count:)`, not to `reloadKeymapDiagnostics()` — that is what keeps an app-wide reload
  from bannering every window, and it keeps `agtermctl keymap reload` silent on success. The one success
  confirmation is the Settings reload BUTTON, which posts its own toast at its own call site.
  ...
  unresolved button into "nothing reloads at all".
  A Settings-page reload reports through the Key Mapping page's Diagnostics group, not a toast — an
  `AdwToast` lands on the window content while Settings is an `AdwPreferencesDialog` presented over it.
  The same cache feeds the Linux action palette's custom rows (`keymap.commands` in `Palette.swift`),
  never a fresh disk parse, so the shortcut column cannot advertise a chord dispatch does not have.
  Pinned by two AT-SPI checks: `check_keymap_reload_fanout` and `check_keymap_error_banner`.
