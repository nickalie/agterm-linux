...
extension GhosttySurfaceView {
    ...
    /// (`onUserInputStatusKeystroke`). Always plain typing: a multi-line insert lands as a paste, not as Return,
    /// and an AX insert is never the Escape/Ctrl-C interrupt that clears an ACTIVE glyph, so it clears completed
    /// and answers blocked into active, and only when the Status reset setting moves on the first key.
    private func insertFromAccessibility(_ text: String) {
        onUserInput?()
        onUserInputStatusKeystroke?(.other)
