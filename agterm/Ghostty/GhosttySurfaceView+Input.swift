...
extension GhosttySurfaceView {
    ...
    override func keyDown(with event: NSEvent) {
        ...
        onUserInput?()
        // a keystroke moves an attention glyph: blocked/completed as the Status reset setting says — an answered
        // block becoming active, not idle — and active clears ONLY on an interrupt (Escape or Ctrl-C), so typing
        // while the agent works keeps the "working" glyph but cancelling a pending prompt drops it. Claude Code
        // treats Ctrl-C like Esc for dismissing a prompt, yet neither fires a hook and a cancelled prompt can
        // still read active (its blocked notification lands seconds later), so this is the only signal that
        // drops the stale glyph. fire UNCONDITIONALLY with the classified key: the pane-scoped decision belongs
        // to AgentIndicator.afterKeystroke, so the scratch (no view.session) transitions too and a background
        // pane's block survives foreground typing.
        onUserInputStatusKeystroke?(classifyKeystroke(event))
