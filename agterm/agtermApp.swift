...
struct agtermApp: App {
    ...
    /// and this closure applies host-free `AgentIndicator.afterKeystroke(pane:keystroke:reset:)` only when the
    /// keystroke's OWN pane owns the status under the Status reset setting, read live from `GhosttyApp` so a
    /// Settings change applies to the next key. A block set from a background pane survives typing elsewhere.
