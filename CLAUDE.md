...
- `keymap.md`: keymap parser, built-ins, custom commands/tokens, reload/edit, and the Linux dispatch,
  keyboard-layout policy, and app-wide reload seam.
...
- `main-loop.md`: the GTK/Linux main-loop contract — deferred main-actor work through `MainTimer`,
  `runOnMain` hops, and the `withFakeMainTimer` test rule.
