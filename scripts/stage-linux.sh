...
cp -R "$ROOT/agterm/Resources/hud" "$DEST/share/agterm/hud"
cp -R "$ROOT/plugins/agterm/skills/agterm" "$DEST/share/agterm/agent-skill"
printf '%s\n' "${AGTERM_PACKAGE_VERSION:-dev}" > "$DEST/share/agterm/VERSION"
# diagnostics for `agtermctl version`; a checkout git cannot answer stages nothing rather than "unknown"
COMMIT="${AGTERM_PACKAGE_COMMIT:-$(git -C "$ROOT" rev-parse --short HEAD 2>/dev/null || true)}"
[[ -n "$COMMIT" ]] && printf '%s\n' "$COMMIT" > "$DEST/share/agterm/COMMIT"
